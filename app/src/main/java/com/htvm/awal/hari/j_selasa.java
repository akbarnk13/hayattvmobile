...
public class j_selasa extends AppCompatActivity {
    public DatabaseReference Dbkoneksi;
    public ListView listviewjadwalminggu;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_jselasa);

        Toolbar toolbar = findViewById(R.id.toolbarr);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        Dbkoneksi = FirebaseDatabase.getInstance().getReference().child("Selasa");
